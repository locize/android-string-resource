    ...
    if (typeof resources[key] !== 'string' && Array.isArray(resources[key])) {
      ...
      if (/\.(zero|one|two|few|many|other)$/.test(key)) {
        ...
        foundBaseElement.elements.push({
          ...
          elements: [textOrCData(resources[key])]
        })
      } else if (/\.\d+$/.test(key)) {
        const lastDotIdx = key.lastIndexOf('.')
        const baseKey = key.substring(0, lastDotIdx)
        ...
        if (resources[key].indexOf('<xliff:g id="') > -1) {
          ...
          stringElements.push({
            ...
            elements: [
              textOrCData(resources[key])
            ]
          })
        }
      }

      if (resources[key].indexOf('<xliff:g id="') > -1 && !declAttr['xmlns:xliff']) {
        declAttr['xmlns:xliff'] = 'urn:oasis:names:tc:xliff:document:1.2'
      }
    }
    ...
    if (typeof resources[key] === 'object' && typeof resources[key].value === 'string' && typeof resources[key].comment === 'string') {
      if (/\.(zero|one|two|few|many|other)$/.test(key)) {
        ...
        foundBaseElement.elements.push({
          ...
          elements: [textOrCData(resources[key].value)]
        })
      } else if (/\.\d+$/.test(key)) {
        const lastDotIdx = key.lastIndexOf('.')
        const baseKey = key.substring(0, lastDotIdx)
        ...
        if (resources[key].value.indexOf('<xliff:g id="') > -1) {
          ...
          const ele = {
            ...
            elements: [
              textOrCData(resources[key].value)
            ]
          }
