  ...
  Object.keys(resources).forEach((key) => {
    ...
    if (typeof resources[key] === 'string') {
      if (/\.(zero|one|two|few|many|other)$/.test(key)) {
        ...
        let foundBaseElement = asrJs.plurals.find((ele) => ele.$.name === baseKey)
        if (!foundBaseElement) {
          foundBaseElement = {
            $: {
              name: baseKey
          ...
          asrJs.plurals.push(foundBaseElement)
        }
        foundBaseElement.item.push({
          $: {
            quantity: plForm
          },
        ...
        let foundBaseElement = asrJs['string-array'].find((ele) => ele.$.name === baseKey)
        if (!foundBaseElement) {
          foundBaseElement = {
            $: {
              name: baseKey
