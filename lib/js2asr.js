...
const js2asrClb = (resources, opt, cb) => {
  ...
  Object.keys(resources).forEach((key) => {
    ...
    if (typeof resources[key] === 'object' && typeof resources[key].value === 'string' && typeof resources[key].comment === 'string') {
      if (/\.(zero|one|two|few|many|other)$/.test(key)) {
        ...
        let foundBaseElementWithComment = arrayElements.find((ele) => ele.attributes.name === baseKey)
  ...
  arrayElements.forEach((ele) => {
    if (ele.elements.length > 1 && ele.elements.filter((e) => e).length < ele.elements.length) {
      const subElements = ele.elements.filter((e) => e)
  ...
  // arrayElements.forEach((ele) => {
  //   for (let index = 0; index < ele.elements.length; index++) {
  //     if (!ele.elements[index]) {
  //       // fill empty elements
  //       ele.elements[index] = {
  //         type: 'element',
  //         name: 'item',
